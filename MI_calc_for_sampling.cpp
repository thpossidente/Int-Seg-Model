...
// [[Rcpp::export]]
List averageMIperClusterSampling(List inputMatrices, int windowSize){
  ...
  NumericMatrix pixelPairs(num_pixel_combos * num_matrices, 2);    // Initialize matrix to store pixel pairs to calculate MI with
      ...
      for(int r=0; r<((inputMatrices.size()));r++){               // for each matrix in the vector of matrices
        ...
        for(int w=0; w<(windowSize);w++){
          for(int d=0; d<(windowSize);d++){
            matClusterVec[counter1] = mat( w , d );
            counter1 += 1;
          }
        }
